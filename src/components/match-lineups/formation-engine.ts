...
export type TacticalLine = 'GK' | 'DEF' | 'HOLD' | 'MID' | 'AM' | 'FWD';
export type SlotKey =
    | 'GK'
    | 'LB' | 'RB' | 'CB1' | 'CB2' | 'CB3'
    | 'LWB' | 'RWB'
...
export type BuildStrategy = 'preset' | 'numeric' | 'detailed' | 'fallback';
...
// ─── Constants ──────────────────────────────────────────
export const LINE_Y: Record<TacticalLine, number> = {
    GK: 92,
    DEF: 75,
    HOLD: 61,
    MID: 49,
...
export const DISTRIBUTED_X: Record<number, number[]> = {
    1: [50],
    2: [36, 64],
    3: [22, 50, 78],
    4: [10, 36, 64, 90],
...
export const SLOT_ASSIGNMENT_PRIORITY: SlotKey[] = [
    'GK',
    'ST', 'ST1', 'ST2',
    'CAM',
    'CDM', 'CDM1', 'CDM2',
...
export const FORMATION_ALIASES: Record<string, string> = {
    '4-3-3': '4-3-3',
    '4-4-2': '4-4-2',
    '4-4-1-1': '4-4-1-1',
    '4-2-3-1': '4-2-3-1',
...
export const classifyPosition = (pos: string): PosGroup => {
...
export const formatSoccerMinute = (value: string): string => {
...
export const getDistributedX = (count: number, index: number): number => {
...
export const getFormationParts = (formation: string | null): number[] =>
    typeof formation === 'string'
        ? formation.split('-').map((part) => Number(part)).filter((part) => part > 0)
        : [];

export const getPresetFormation = (formation: string | null): string | null => {
...
export const getBaseLineFromPosition = (player: LineupPlayer): TacticalLine => {
...
export const getPositionalDepth = (player: LineupPlayer): number => {
    const code = String(player.positionCode || '').toUpperCase();
...
export const inferPresetFormation = (starters: LineupPlayer[]): string | null => {
...
// ─── Formation scoring / inference from places ──────────
...
export const inferPresetFormationFromPlaces = (starters: LineupPlayer[]): string | null => {
...
export const getEffectiveFormation = (formation: string | null, starters: LineupPlayer[]): string | null => {
...
export const buildDetailedRows = (formation: string | null, starters: LineupPlayer[]): FormationRow[] => {
...
export const buildNumericFormationRows = (formation: string, starters: LineupPlayer[]): { rows: FormationRow[]; confident: boolean } | null => {
...
export const buildFormationFallbackRows = (formation: string | null, starters: LineupPlayer[]): FormationRow[] => {
